...
def criar_profissional_tste():
    ...
    try:
        ...
        if func_row:
            ...
            # Tentativa com data_nascimento
            try:
                cur.execute("""
                    INSERT INTO funcionarios 
                    (nome, email, especialidade, unidade_atendimento, ativo, atendimento, situacao, cpf, telefone, data_nascimento)
                    VALUES 
                    ('Dra. Ana Souza', %s, 'Psicólogo', 'USF Centro', TRUE, TRUE, 'Ativo', '12345678900', '81999999999', '1985-05-15')
                    RETURNING id
                """, (email,))
                func_id = cur.fetchone()[0]
                conn.commit()
            except Exception as e_insert:
                 print(f"Erro no INSERT funcionario: {e_insert}")
                 return
